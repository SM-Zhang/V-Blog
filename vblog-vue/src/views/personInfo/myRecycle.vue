            ...
            <el-table-column label="操作" width="145">
                <template slot-scope="scope">
                    <el-button
                        size="mini"
                        type="success"
                        @click="handleRecycleButton(scope.row.aid)">恢复</el-button>
                    <el-button
                        size="mini"
                        type="danger"
...
<script>
import { getAllArticles, delArticleById, delUpArticleStateById } from '@/api/articleApi'
...
export default {
    ...
    methods: {
        ...
        // 博客恢复至草稿箱
        handleRecycleButton(id){
            this.$confirm('此操作将恢复至草稿箱', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then( () => {
                delUpArticleStateById(id, 2).then( response => {
        ...
        handleDeleteButton(id){
            this.$confirm('此操作将永久删除当前信息', '提示', {
                ...
                // 删除,此操作不可更改
                delArticleById(id).then(response => {
                    const resp = response.data;
                    if(resp.flag){
                         this.fetchData();
                    }
